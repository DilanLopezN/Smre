...
export const useConversationObjectives = () => {
  ...
  const useConversationObjectivesLocaleKeys =
    localeKeys.dashboard.categorizationDashboard.hooks.useConversationObjectives;
  ...
  const fetchConversationObjective = useCallback(async () => {
    try {
      ...
      notifyError(t(useConversationObjectivesLocaleKeys.createObjectivesError));
      setFetchConversationObjectivesError(error as ApiError);
      setIsFetchingConversationObjectives(false);
      return false;
    }
  }, [useConversationObjectivesLocaleKeys.createObjectivesError, t, workspaceId]);
