...
import type { DataType, ProcedureListProps } from './interfaces';
...
export const ProcedureList = ({
  ...
  const { data, isLoading, fetchConfirmationEntity } = useHealthEntities({
    ...
    entityType: HealthEntityType.procedure,
  });
  ...
  const handleRowClick = (recordKey: string) => {
    ...
    const specialityArrayIndex = selectedProcedureList.findIndex(
      (procedureList) => procedureList === recordKey
    );

    if (specialityArrayIndex >= 0) {
      newProcedureList = selectedProcedureList.filter(
        (procedureList) => procedureList !== recordKey
      );
    } else {
      newProcedureList = [...selectedProcedureList, recordKey];
    }

    setSelectedProcedureList(newProcedureList);
  };
  ...
  const columns: ColumnsType<DataType> = [
    {
      title: t(filtersModalLocaleKeys.titleProcedureList),
      dataIndex: 'description',
      render: (text) => <span title={text}>{text}</span>,
    },
  ];

  const formattedData = data?.data.map((procedure) => {
    return {
      key: procedure.code,
  ...
  const footerActions = () => {
    return (
      ...
      >
        {t(filtersModalLocaleKeys.procedureNameResetButton)}
      </a>
    );
  };
  ...
  return (
    <Container>
      <Row gutter={[16, 16]}>
        <Col span={24}>
          <Input.Search
            placeholder={t(filtersModalLocaleKeys.procedureNameSearchInputPlaceholder)}
            allowClear
        ...
        <Col span={24}>
          <Table
            ...
            rowSelection={{
              type: 'checkbox',
              selectedRowKeys: selectedProcedureList,
              onChange: (newSelectedRowKeys) => {
                setSelectedProcedureList(newSelectedRowKeys as string[]);
              },
